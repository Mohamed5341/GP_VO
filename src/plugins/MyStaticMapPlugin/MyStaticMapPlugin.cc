...
namespace gazebo
{
  ...
  /// \brief Private data class for MyStaticMapPlugin
...
GZ_REGISTER_WORLD_PLUGIN(MyStaticMapPlugin)
...
bool DownloadFile(const std::string &_url, const std::string &_outputFile)
{
  ...
  if (!fp)
  {
    std::cout << "Could not download model[" << _url << "] because we were"
  ...
  if (success != CURLE_OK)
  {
    std::cout << "Error in REST request" << std::endl;
...
/////////////////////////////////////////////////
MyStaticMapPlugin::MyStaticMapPlugin()
  : dataPtr(new StaticMapPluginPrivate)
{
}

/////////////////////////////////////////////////
void MyStaticMapPlugin::Load(physics::WorldPtr _world, sdf::ElementPtr _sdf)
{
...
void deg2num(float lat_deg, float long_deg, unsigned int zoom, unsigned int *xtile, unsigned int *ytile)
{
    unsigned int powzoom = pow(2.0, zoom);
    float lat_rad = lat_deg*M_PI/180.0;

    float xtilef = (long_deg + 180.0) / 360.0 * powzoom;
    float ytilef = (1.0 - log(tan(lat_rad)+ 1/cos(lat_rad)) / M_PI)/2.0*powzoom;

    *xtile = int(xtilef);
    *ytile = int(ytilef);

    const int tilesize = 256;
    int x_off = int((xtilef - *xtile) * tilesize);
    int y_off = int((ytilef - *ytile) * tilesize);
}
    ...
    const std::string &_apiKey, const std::string &_saveDirPath)
{
  ...
  // file to write data
  std::ofstream myfile;
  myfile.open(_saveDirPath + "/tiles_list.csv");
  myfile << "path, url\n";

  // download map tiles using google static map API
  std::string url = "http://mt1.google.com/vt/lyrs=s";
  ...
  for (unsigned int i = 0; i < yNumTiles; ++i)
  {
    for (unsigned int j = 0; j < xNumTiles; ++j)
    {
      ...
      deg2num(latLon.LatitudeReference().Degree(), latLon.LongitudeReference().Degree(), _zoom, &xtile, &ytile);
      ...
      filename << "tile_"
               ...
               << "_" << latLon.LongitudeReference().Degree() << ".jpeg";
      std::string fullPath = _saveDirPath + "/" + filename.str();
      //DownloadFile(fullURL, fullPath);
      myfile << fullPath << "," << fullURL << "\n";
      std::cout << "Downloading map tile: " << k2 << "\t" << filename.str() << std::endl;
